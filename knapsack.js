...
const naiveKnapsack = (items, capacity) => {
  let [i, result] = [items.length, { value: 0, size: 0, chosen: [] }];
  ...
  while (i) {
    // decrement
    i--;
    const val = result.size + items[i].size;
    // add to the value and size while the capacity is not reached
    if (val < capacity) {
      // add the value
      console.log(i);
      result.value += items[i].value;
      result.size += items[i].size;
      result.chosen.push(i + 1);
    } else {
      // do aa for loop to check if we can add
    }
  }
  console.log(result);
  // recursive function
