...
def fractional_knapsack(items, weight_capacity):
    ...
    for item in items_sorted:
        if weight_current + weight(item) < weight_capacity:
            ...
            print(
                "added item with (value, weight) = (%s, %s)"
            ...
            print(
                "added PORTION %.2f of item with (value, weight) = (%s, %s), contributing only (%s, %.2f)"
                % (
                    portion,
                    value(item),
                    weight(item),
...
def discrete_knapsack(items, size, weight_capacity):
    ...
    if weight_capacity - weight(item) >= 0:
        items_keep = discrete_knapsack(
            items, size - 1, weight_capacity - weight(item)
        ) + [item]
...
def discrete_knapsack_memo(items, size, weight_capacity, dptable):
    ...
    if weight_capacity - weight(item) >= 0:
        if dptable[size - 1][weight_capacity - weight(item)] is not None:
            ...
            items_keep = discrete_knapsack_memo(
                items, size - 1, weight_capacity - weight(item), dptable
            ) + [item]
...
def discrete_knapsack_memo_toplevel(items, size, weight_capacity):
    dptable = [
        [None for j in range(weight_capacity + 1)] for i in range(len(items) + 1)
    ]
...
def discrete_knapsack_dp_trace_solution(dptable, items):
    ...
    while item_idx != -1:
        ...
        if weight_idx > next_weight_idx:
            print(
                "took item with (value, weight) = (%d, %d)"
                % (value(items[item_idx]), weight(items[item_idx]))
            )
...
def discrete_knapsack_dp(items, weight_capacity):
    dptable = [
        [[-1, (-1, -1)] for j in range(weight_capacity + 1)] for i in range(len(items))
    ]
    VALUE = 0
    PARENT = 1
        ...
        for weight_idx in range(len(dptable[item_idx])):
            ...
            if item_idx == 0:
                ...
                if keepitem_weight_idx >= 0:
                    value_if_keepitem = dptable[item_idx - 1][keepitem_weight_idx][
                        VALUE
                ...
                if value_if_leaveitem >= value_if_keepitem:
                    ...
                    dptable[item_idx][weight_idx][PARENT] = (
                        item_idx - 1,
                        keepitem_weight_idx,
                    )
    ...
    print(
        "number of iterations = (weight_capacity+1) * len(items) = %d"
        % ((weight_capacity + 1) * len(items))
    )
    print(
